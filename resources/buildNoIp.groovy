podTemplate(containers: [
  ...
  node(POD_LABEL) {
    stage('Build jnlp-agent') {
      git 'https://github.com/coppit/docker-no-ip'
      container('k3c') {
        stage('Build in k3c container') {
          sh "k3c build -f Dockerfile -t stephenshelton/no-ip:0.0.${BUILD_NUMBER} ."
          ...
          while(!imageOutput.contains("stephenshelton/no-ip")) {
